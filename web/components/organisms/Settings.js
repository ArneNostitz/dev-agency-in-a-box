// Organism — Settings. Extracted from web/settings.js; logic unchanged.
// The settings sheet, GitHub connection (OAuth device flow), models/runners modal, provider editor,
// add-provider flow, operations panel, and the admin team panel.
//
// Imports atoms + molecules + lib from their new (split) locations. SecretField is now the shared
// molecule. OB_PROVIDERS comes from data/providers.js (breaking the old import cycle from onboarding).
...
import { providerModelOptions } from "../../lib/model-logic.js";
 ...
 */
export function ModelsPanel({ providers, globalModel, setGlobalModel, autoSwitch, setAutoSwitch, chain, setChain, onOpenModels }) {
...
export function Operations({ meta, values, reload }) {
...
export function ProviderEditor({ provider, all, onClose, onSave }) {
...
export function AddProvider({ existing, onClose, onSaved }) {
...
export function Admin({ users, webhookSecretSet, reload }) {
