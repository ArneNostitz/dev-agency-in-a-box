...
function App() {
  ...
  const [view, setView] = useState(() => { try { const v = localStorage.getItem("view"); return (v === "list" || v === "board") ? v : "list"; } catch (e) { return "list"; } });
  const [chatOpen, setChatOpen] = useState(() => { try { return localStorage.getItem("chatOpen") === "1"; } catch (e) { return false; } });
  const setChatOpenP = (v) => { setChatOpen(v); try { localStorage.setItem("chatOpen", v ? "1" : "0"); } catch (e) {} };
  ...
  const act = {
    ...
    hold(repo, number, steer) { return guard("hold", repo, number, () => { override(repo, number, { blocked: "held" }); return api("/hold", { repo, number, body: steer || "" }).then(() => toast("Interrupt queued — pausing at the next safe break")).catch(() => toast("Couldn’t interrupt", "error")).then(load); }); },
    ...
    resetIssue(repo, number) { return guard("reset", repo, number, () => { override(repo, number, { state: "planned", active: false }); return api("/reset-issue", { repo, number }).then(() => toast("Issue fully reset")).catch((e) => toast((e && e.message) || "Couldn't reset", "error")).then(load); }); },
    // Inbox → Planned: same endpoint as cancel ("reset to Planned regardless of state"), distinct
    // busy-key + toast for the Inbox card's "Plan" button.
    plan(repo, number) { return guard("plan", repo, number, () => { override(repo, number, { state: "planned", active: false }); return api("/cancel", { repo, number }).then(() => toast("Moved to Planned")).catch((e) => toast((e && e.message) || "Couldn’t plan", "error")).then(load); }); },
    ...
    startChildren(repo, number) { return guard("startChildren", repo, number, () => api("/start-children", { repo, number }).then((r) => { toast(r && r.started ? `Started sub-issue #${r.started}` : "Started next sub-issue"); }).catch((e) => { const msg = (e && e.message) || "Couldn't start sub-issue"; toast(msg, "error"); }).then(load)); },
    epicPlay(repo, number) { return guard("epicPlay", repo, number, () => api("/epic-play", { repo, number }).then((r) => toast(r && r.started ? `▶ Playing epic — started #${r.started}; each merge starts the next` : "▶ Playing epic")).catch((e) => toast((e && e.message) || "Couldn't play the epic", "error")).then(load)); },
    epicPause(repo, number) { return guard("epicPause", repo, number, () => api("/epic-pause", { repo, number }).then(() => toast("Epic auto-run paused")).catch((e) => toast((e && e.message) || "Couldn't pause", "error")).then(load)); },
    ...
    <div class="app">
      ...
      <div class="content">
        ${repos.length ? (() => {
          ...
          const toolbar = html`<${StatStrip} counts=${statCounts} statFilter=${statFilter} setStatFilter=${setStatFilter} compact=${true}/>`;
          const listLeaf = html`<${ProgressTable} issues=${shown} repos=${repos} repoFilter=${repoFilter} compact=${detailOpen} openKey=${openKey} onOpen=${(i) => setOpenKey(i.repo + "#" + i.number)} onAddIssue=${(r) => openComposer(r)} onAnalyze=${(r) => act.audit(r)} auditRepos=${auditRepos} act=${act} data=${data} statFilter=${statFilter} setStatFilter=${setStatFilter} toolbarExtra=${toolbar}/>`;
          const boardLeaf = html`<${Board} issues=${shown} repos=${repos} repoFilter=${repoFilter} tab=${tab} isDesktop=${isDesktop} onOpen=${(i) => setOpenKey(i.repo + "#" + i.number)} onOpenChild=${openIssue} onAddRepo=${() => setSheet("addrepo")} onAddIssue=${(r) => openComposer(r)} onAnalyze=${(r) => act.audit(r)} auditRepos=${auditRepos} act=${act} data=${data} onOpenModels=${() => setSheet("models")}/>`;
          const chatLeaf = html`<${Orchestrator} repos=${repos} repoFilter=${repoFilter} setRepoFilter=${setRepoFilter} reload=${load} onOpenIssue=${openIssue} issues=${issues} onAnalyze=${(r) => act.audit(r)} auditRepos=${auditRepos} onClose=${() => setChatOpenP(false)}/>`;
          const detailLeaf = open ? html`<${Detail} key=${openKey} docked=${true} issue=${open} activity=${activity} act=${act} isDesktop=${isDesktop} startError=${detailError} onClose=${() => { setOpenKey(null); setDetailError(null); }} onOpenIssue=${openIssue} data=${data} isOnline=${isOnline} onQueueComment=${oqPush} onOpenModels=${() => setSheet("models")}/>` : null;
          return html`<${Workspace} vw=${vw} view=${view} mainView=${view === "board" ? boardLeaf : listLeaf} chat=${chatLeaf} detail=${detailLeaf} chatOpen=${chatOpen} setChatOpen=${setChatOpenP} detailOpen=${detailOpen}/>`;
        })() : html`<${Board} issues=${shown} repos=${repos} repoFilter=${repoFilter} tab=${tab} isDesktop=${isDesktop} onOpen=${(i) => setOpenKey(i.repo + "#" + i.number)} onOpenChild=${openIssue} onAddRepo=${() => setSheet("addrepo")} onAddIssue=${(r) => openComposer(r)} onAnalyze=${(r) => act.audit(r)} auditRepos=${auditRepos} act=${act} data=${data} onOpenModels=${() => setSheet("models")}/>`}
      </div>
