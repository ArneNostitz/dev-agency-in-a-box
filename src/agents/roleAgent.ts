...
import { recentLessons, recordTokens, getProviders, getRoleModels, setSession, getIssueModelOverride } from "../store.js";
...
function resolveRoute(role: RoleName, repo: string, issueNumber: number): { model: string; env: Record<string, string> } | null {
  ...
  const assignment = issueOverride ?? getRoleModels()[role];
